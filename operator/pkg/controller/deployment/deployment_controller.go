...
func (r *ReconcileDeployment) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if checkAlamedaAnnotationIsPresent(annotations) {
		//TODO: Handle the deployment with alameda annotations.
		log.Printf("Handling deployment %s/%s with alameda annotations.\n", instance.GetNamespace(), instance.GetName())
	}

	return reconcile.Result{}, nil
}

func checkAlamedaAnnotationIsPresent(annotations map[string]string) bool {

	reg, _ := regexp.Compile(fmt.Sprintf("^%s", AlamedaAnnotationPrefix))
