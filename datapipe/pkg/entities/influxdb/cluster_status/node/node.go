...
//raw data
var (
	NodeDatabaseName    = "alameda_cluster_status"
	NodeMeasurementName = "node"
	NodeColumns         = []string{
		//tag
		NodeName,
	...
	NodeColumnsTypes = make([]commonAPI.ColumnType, 0)
	NodeDataTypes    = make([]commonAPI.DataType, 0)

	NodeColIndexMap = map[string]int{}
)

func init() {
	for index, value := range NodeColumns {
		if index < len(NodeTags)-1 {
			NodeColumnsTypes = append(NodeColumnsTypes, commonAPI.ColumnType_COLUMNTYPE_TAG)
		} else {
			NodeColumnsTypes = append(NodeColumnsTypes, commonAPI.ColumnType_COLUMNTYPE_FIELD)
		}

		NodeDataTypes = append(NodeDataTypes, commonAPI.DataType_DATATYPE_STRING)
		NodeColIndexMap[value] = index
	}

	NodeDataTypes[NodeColIndexMap[NodeInCluster]] = commonAPI.DataType_DATATYPE_BOOL
	NodeDataTypes[NodeColIndexMap[NodeCPUCores]] = commonAPI.DataType_DATATYPE_INT64
	NodeDataTypes[NodeColIndexMap[NodeMemoryBytes]] = commonAPI.DataType_DATATYPE_INT64
	NodeDataTypes[NodeColIndexMap[NodeCreateTime]] = commonAPI.DataType_DATATYPE_INT64
