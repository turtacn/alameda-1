...
func (p *NodeMetrics) CreateMetrics(metrics DaoMetricTypes.NodeMetricMap) error {
	...
	err := nodeCpuRepo.CreateMetrics(metrics.GetSamples(DatahubMetric.TypeNodeCPUUsageSecondsPercentage))
	if err != nil {
	...
	err = nodeMemoryRepo.CreateMetrics(metrics.GetSamples(DatahubMetric.TypeNodeMemoryUsageBytes))
	if err != nil {
		scope.Error(err.Error())
		return err
	}
