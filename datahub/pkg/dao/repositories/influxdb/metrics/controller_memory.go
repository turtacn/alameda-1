...
func NewControllerMemoryRepositoryWithConfig(influxDBCfg InternalInflux.Config) *ControllerMemoryRepository {
	return &ControllerMemoryRepository{
		influxDB: &InternalInflux.InfluxClient{
...
func (r *ControllerMemoryRepository) CreateMetrics(ctx context.Context, metrics []DaoMetricTypes.ControllerMetricSample) error {
	...
	for _, metric := range metrics {
		...
		for _, sample := range metric.Metrics {
			...
			point, err := InfluxClient.NewPoint(string(ControllerMemory), tags, fields, sample.Timestamp)
			if err != nil {
...
func (r *ControllerMemoryRepository) read(ctx context.Context, request DaoMetricTypes.ListControllerMetricsRequest) (DaoMetricTypes.ControllerMetricMap, error) {

	statement := InternalInflux.Statement{
		Measurement:    ControllerMemory,
...
func (r *ControllerMemoryRepository) steps(ctx context.Context, request DaoMetricTypes.ListControllerMetricsRequest) (DaoMetricTypes.ControllerMetricMap, error) {
	...
	statement.SetOrderClauseFromQueryCondition()
	statement.SetLimitClauseFromQueryCondition()
	statement.SetFunction(InternalInflux.Select, "MAX", string(EntityInfluxMetric.ControllerValue))
	cmd := statement.BuildQueryCmd()
