...
var (
	scope = Log.RegisterScope("metric_db_measurement", "metric DB measurement", 0)
)
...
func (r *ContainerCpuRepository) CreateMetrics(metrics []*DaoMetricTypes.ContainerMetricSample) error {
	...
	for _, metricSample := range metrics {
		for _, metric := range metricSample.Metrics {
			...
			point, err := InfluxClient.NewPoint(string(ContainerCpu), tags, fields, metric.Timestamp)
			if err != nil {
		...
		for i := 0; i < result.GetGroupNum(); i++ {
			...
			for j := 0; j < group.GetRowNum(); j++ {
				...
				if row["value"] != "" {
					...
					containerMetric.AddSample(DatahubMetric.TypeContainerCPUUsageSecondsPercentage, sample)
				}
			}
...
func (r *ContainerCpuRepository) steps(request DaoMetricTypes.ListPodMetricsRequest) ([]*DaoMetricTypes.ContainerMetric, error) {
	containerMetricList := make([]*DaoMetricTypes.ContainerMetric, 0)
